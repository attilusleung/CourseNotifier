...
argparser = ArgumentParser(description='Checks availability of Cornell classes')
# argparser.add_argument('--classes', nargs='+', default=None, help='Read from list of class codes')
...
def intersect_class_dicts(openings, class_dict):
    return dict([(x, [y for y in openings[x] if y.number in class_dict[x]]) for x in openings.keys() if x in class_dict])

def minus_dicts(dict1, dict2):
    return dict([(x, [y for y in dict1[x] if y not in dict2[x]]) if x in dict2 else (x, dict1[x]) for x in dict1.keys()])

def str_dict_items(d):
    return dict([(k, "\n" + "\n".join([str(i) for i in v])) for k, v in d.items()])
...
while(True):
    try:
        ...
        classes_open = dict([(c, intersect_class_dicts(s, classes[c])) for c, s in openings.items() if c in classes])
        # print(classes_open)

        # if any([all([s for s in c.values()]) for c in classes_open.values() if c]):
        ...
        if classes_open != prev:
            ...
            changes = dict([(c, minus_dicts(s, prev[c])) if c in prev else (c, s) for c, s in classes_open.items()])
            msg_builder.clear()
        ...
        message = message_client.messages.create(
                body= \
                f"Unhandled exception during main loop \n\n{format_exc()}",
                from_=confparser['TWILIO']['SOURCE_PHONE'],
                to=confparser['TWILIO']['DEST_PHONE']
                )
